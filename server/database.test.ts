...
describe('production data migrations', () => {
  ...
  it('deletes an account and all server-side learning data', () => {
    ...
    db.saveTunerReading('usr_delete', {
      ...
      buttonId: 'button-1', row: 1, buttonIndex: 1, hand: 'right', direction: 'push', expectedMidi: 60, detectedMidi: 60,
  ...
  it('archives tuner readings by campaign and retrieves the latest one', () => {
    ...
    const base = {
      ...
      buttonIndex: 3, hand: 'left' as const, direction: 'push' as const, expectedMidi: 55, detectedMidi: 65, frequency: 349.8,
    ...
    expect(db.listTunerReadings('usr_tuner')).toMatchObject([{ id: 'reading-new', cents: 3, hand: 'left', outcome: 'corrected' }]);

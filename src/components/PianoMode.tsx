...
import { Check, ChevronLeft, ChevronRight, Clock3, Keyboard, Mic2, Piano, Play, Repeat2, RotateCcw, Target, Usb, X } from 'lucide-react';
...
export function PianoMode({ keyboardSize, input, onSessionUpdate, view, stats, onNavigate }: PianoModeProps) {
  ...
  if (screen === 'home' && view === 'home') return <main className="page-content piano-page piano-dashboard">
    <header className="piano-hero"><span><Piano /></span><div><small>TON PARCOURS PIANO</small><h1>Reprends là où tu en étais</h1><p>Une suggestion courte, puis quelques minutes de pratique ciblée.</p></div><button type="button" onClick={() => { setExercise(PIANO_EXERCISES[0]); setScreen('exercise'); }}><Play /> Suggestion du jour</button></header>
    <section className="piano-dashboard-grid"><article className="daily-piano-card"><small>AUJOURD’HUI · TRÈS SIMPLE</small><h2>Trois petits pas</h2><p>Do, Ré et Mi à tempo calme pour retrouver tes repères.</p><button type="button" onClick={() => { setExercise(PIANO_EXERCISES[0]); setScreen('exercise'); }}>Commencer <ChevronRight /></button></article><article className="piano-recommendation"><Target /><div><small>CONSEIL PERSONNALISÉ</small><strong>{stats?.hasData ? 'Consolide les notes encore hésitantes' : 'Commence par les positions de la main droite'}</strong><p>{stats?.hasData ? 'Ralentis à 80 % et rejoue les passages marqués en rouge.' : 'Le premier exercice utilise seulement Do, Ré et Mi.'}</p></div></article></section>
  ...
  return <main className="piano-player">
    ...
    {result ? <section className="piano-results"><Check /><h1>Exercice terminé</h1><div><article><strong>{result.correct}</strong><span>correctes</span></article><article><strong>{result.missed}</strong><span>ratées</span></article><article><strong>{result.averageDelay} ms</strong><span>retard moyen</span></article><article><strong>{result.rhythmAccuracy} %</strong><span>précision rythme</span></article></div><b>{result.global} / 100</b><p>{result.advice}</p><button type="button" className="primary-button" onClick={start}><RotateCcw /> Recommencer</button></section> : <>
      <section className="piano-play-options"><label>Mode<select value={playMode} disabled={playing} onChange={(event) => setPlayMode(event.target.value as 'learning' | 'game')}><option value="learning">Apprentissage · attend la note</option><option value="game">Jeu · tempo et score</option></select></label><label>Vitesse <strong>{tempoPercent} %</strong><input type="range" min="50" max="100" step="10" value={tempoPercent} disabled={playing} onChange={(event) => setTempoPercent(Number(event.target.value))} /></label></section>
      <section className="piano-roll"><div className="hit-line" />{exercise.notes.map((note, index) => { const elapsed = playing ? (performance.now() - startRef.current) / beatMs : 0; return <i key={index} className={`${index === activeIndex ? 'is-active' : ''} ${judgedRef.current.has(index) ? 'is-judged' : ''}`} style={{ '--lane': note.midi - 60, '--duration': note.duration, transform: `translateY(${(elapsed - note.beat) * 72}px)` } as React.CSSProperties}><span>{frenchNote(note.midi).replace(/\d$/, '')}</span></i>; })}</section>
      <PianoKeyboard size={keyboardSize} expected={expected} played={played} error={errorKey} onPlay={judge} />
      <footer><span>{input === 'midi' ? <Usb /> : input === 'microphone' ? <Mic2 /> : <Keyboard />} {inputMessage}</span><button type="button" className="primary-button" disabled={playing} onClick={start}><Play /> {playing ? 'En cours…' : 'Démarrer'}</button><progress value={activeIndex + 1} max={exercise.notes.length} /></footer>
    </>}
  </main>;
}

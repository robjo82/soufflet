...
export function AccountPage({ user, accordions, selectedAccordionId, instrumentType, learningInstruments, onInstrumentChange, onUserUpdated, onOpenSettings, onLogout, onAccountDeleted }: AccountPageProps) {
    ...
    <main className="page-content account-page">
      ...
      <div className="account-layout">
        ...
        <aside className="account-sidebar">
          <section className="account-card"><header><span><Music2 /></span><div><h2>Mes parcours</h2><p>{learningInstruments.length === 2 ? 'Accordéon et piano sont activés.' : 'Change de parcours sans perdre tes données.'}</p></div></header><div className="profile-instrument"><button type="button" className={instrumentType === 'accordion' ? 'is-selected' : ''} onClick={() => onInstrumentChange('accordion')}>Accordéon {learningInstruments.includes('accordion') && '✓'}</button><button type="button" className={instrumentType === 'piano' ? 'is-selected' : ''} onClick={() => onInstrumentChange('piano')}>Piano {learningInstruments.includes('piano') && '✓'}</button></div></section>

...
export function AccountPage({ user, accordions, selectedAccordionId, instrumentType, learningInstruments, onInstrumentChange, onLearningInstrumentsChange, onUserUpdated, onOpenSettings, onLogout, onAccountDeleted }: AccountPageProps) {
  ...
  return (
    <main className={`page-content account-page ${learningInstruments.includes('accordion') ? '' : 'piano-only-profile'}`}>
      <header className="page-heading account-heading"><div><span className="eyebrow">Espace personnel</span><h1>Mon compte</h1><p>Gère ton identité, la sécurité et le matériel utilisé avec Soufflet.</p></div></header>

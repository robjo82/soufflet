...
import type { AccordionConfig, GuitarConfig, LeftHandAcousticProfile, PianoConfig, UserAccount } from '../types';
...
export function AccountPage({ user, accordions, selectedAccordionId, pianos = [], selectedPianoId, guitars = [], selectedGuitarId, onUserUpdated, onOpenSettings, onLogout, onAccountDeleted }: AccountPageProps) {
    ...
    <main className="page-content account-page">
      ...
      <div className="account-layout">
        ...
        <aside className="account-sidebar">
          <section className="account-card equipment-card"><header><span><Music2 /></span><div><h2>Mon matériel</h2><p>Instruments et configurations synchronisés avec ton compte.</p></div></header><div className="equipment-list">{instruments.map((accordion) => <article key={accordion.id}><i style={{ background: accordion.color }} /><div><small>{accordion.maker}</small><strong>{accordion.model}</strong><span>{accordion.tuning} · {accordion.rightRows.join('+')} + {accordion.bassCount}</span></div>{accordion.id === selectedAccordionId && <em>Accordéon actif</em>}</article>)}{pianos.map((piano) => <article key={piano.id}><i className="piano-equipment-mark" /><div><small>Piano · {piano.keyboardSize} touches</small><strong>{piano.name}</strong><span>{piano.input === 'midi' ? 'Entrée MIDI' : piano.input === 'microphone' ? 'Écoute au microphone' : 'Clavier d’ordinateur'}</span></div>{piano.id === selectedPianoId && <em>Piano actif</em>}</article>)}{guitars.map((guitar) => <article key={guitar.id}><i className="guitar-equipment-mark" /><div><small>Guitare · {guitar.strings.length} cordes</small><strong>{guitar.name}</strong><span>{guitar.strings.map((string) => string.note).join(' · ')}{guitar.capo ? ` · capo ${guitar.capo}` : ''}</span></div>{guitar.id === selectedGuitarId && <em>Guitare active</em>}</article>)}</div><button type="button" className="account-link" onClick={onOpenSettings}><span>Gérer mes instruments</span><ChevronRight /></button></section>

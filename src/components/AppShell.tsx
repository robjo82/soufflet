    ...
    <div className="app-shell">
      <header className="topbar">
        ...
        <nav className={mobileOpen ? 'is-open' : ''}>
          {NAV.filter((item) => instrumentType === 'accordion' || item.id !== 'game').map(({ id, label, icon: Icon }) => <button type="button" key={id} onClick={() => navigate(id)} className={page === id ? 'is-active' : ''}><Icon />{id === 'tuner' && instrumentType === 'piano' ? 'Entrée piano' : label}</button>)}
        </nav>

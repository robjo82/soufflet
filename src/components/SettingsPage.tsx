...
import type { AccordionConfig, InstrumentType, Notation, PianoConfig } from '../types';
...
interface SettingsPageProps {
  ...
  countIn: boolean;
  apiKey: string;
  onSave: (instrumentType: InstrumentType, accordionId: string, pianoId: string, notation: Notation, countIn: boolean, apiKey: string) => void;
...
export function SettingsPage({ accordions, pianos, instrumentType, selectedId, selectedPianoId, notation, countIn, apiKey, onSave, onCreateAccordion }: SettingsPageProps) {
  const [activeInstrument, setActiveInstrument] = useState(instrumentType);
  const [selected, setSelected] = useState(selectedId);
  const [selectedPiano, setSelectedPiano] = useState(selectedPianoId);
  ...
  return (
    <main className="page-content settings-page">
      <header className="page-heading split-heading"><div><span className="eyebrow">Personnalisation</span><h1>Réglages</h1><p>Ton instrument, tes repères, ton environnement de pratique.</p></div><button type="button" className="primary-button" onClick={() => { onSave(activeInstrument, selected, selectedPiano, nextNotation, nextCountIn, key); setSaved(true); setTimeout(() => setSaved(false), 1600); }}>{saved ? <Check /> : <Save />} {saved ? 'Enregistré' : 'Enregistrer'}</button></header>
      ...
      <div className="settings-content">
        <section className="settings-section" id="settings-instrument"><div className="section-title"><div><span className="eyebrow">Instrument actif</span><h2>Ton instrument</h2></div></div><div className="settings-instrument-tabs"><button type="button" className={activeInstrument === 'accordion' ? 'is-selected' : ''} onClick={() => setActiveInstrument('accordion')}><span className="mini-bellows" /> Accordéon</button><button type="button" className={activeInstrument === 'piano' ? 'is-selected' : ''} onClick={() => setActiveInstrument('piano')}><Piano /> Piano</button></div>{activeInstrument === 'piano' ? <div className="piano-settings-grid"><div className="accordion-list">{pianos.map((item) => <button type="button" key={item.id} className={item.id === selectedPiano ? 'is-selected' : ''} onClick={() => setSelectedPiano(item.id)}><Piano /><span><small>{item.keyboardSize} touches</small><strong>{item.name}</strong><em>{item.input === 'midi' ? 'MIDI' : item.input === 'microphone' ? 'Microphone' : 'Clavier ordinateur'}</em></span>{item.id === selectedPiano && <Check />}</button>)}</div><div className="piano-settings-preview"><Piano /><h3>{pianos.find((item) => item.id === selectedPiano)?.name}</h3><p>Le MIDI permet de reconnaître les accords et les deux mains. Le microphone reste disponible pour travailler une ligne mélodique, note par note.</p><div className="warning-inline"><Mic2 /> Pour une évaluation complète à deux mains, branche un clavier MIDI USB ou Bluetooth.</div></div></div> : <div className="accordion-setting-grid"><div className="accordion-list">{accordions.map((item) => <button type="button" key={item.id} className={item.id === selected ? 'is-selected' : ''} onClick={() => setSelected(item.id)}><span style={{ background: item.color }} /><span><small>{item.maker}</small><strong>{item.model}</strong><em>{item.tuning}</em></span>{item.id === selected && <Check />}</button>)}<button type="button" className="custom-instrument" onClick={openCustomEditor}><span>+</span><strong>Créer une configuration personnalisée</strong></button></div><div className="instrument-preview"><AccordionInstrument config={accordion} direction="push" notation={nextNotation} compact context="settings" showLearningGuides={false} /><p>{accordion.description}</p>{!accordion.verified && <div className="warning-inline"><AlertTriangle /> Cette disposition varie selon l’année. Vérifie-la bouton par bouton avec l’accordeur.</div>}</div></div>}</section>

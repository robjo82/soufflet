...
import { AlertTriangle, Check, ChevronRight, Copy, Eye, EyeOff, KeyRound, Mic2, Palette, Save, ShieldCheck, SlidersHorizontal, Sparkles, Volume2, X } from 'lucide-react';
...
interface SettingsPageProps {
  ...
  notation: Notation;
  apiKey: string;
  onSave: (accordionId: string, notation: Notation, apiKey: string) => void;
...
export function SettingsPage({ accordions, selectedId, notation, apiKey, onSave, onCreateAccordion }: SettingsPageProps) {
  ...
  return (
    <main className="page-content settings-page">
      <header className="page-heading split-heading"><div><span className="eyebrow">Personnalisation</span><h1>Réglages</h1><p>Ton instrument, tes repères, ton environnement de pratique.</p></div><button type="button" className="primary-button" onClick={() => { onSave(selected, nextNotation, key); setSaved(true); setTimeout(() => setSaved(false), 1600); }}>{saved ? <Check /> : <Save />} {saved ? 'Enregistré' : 'Enregistrer'}</button></header>

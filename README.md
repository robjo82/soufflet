...
- Hohner Club I 3D dans l’onboarding, le tutoriel, le tableau de bord, le lecteur, le studio, l’accordeur et les réglages : bois veiné, soufflet en vague avec inertie, 29 boutons synchronisés, guides pédagogiques, cadrages responsive, clavier accessible, repli HTML fidèle et laboratoire complet ;
...
- **Gemini** : le modèle est configurable côté serveur (`GEMINI_MODEL`) et produit des sorties structurées à partir des sources musicales, audio, vidéo et PDF disponibles. Le modèle actuellement déployé est exposé par `/api/health`. Voir [Video understanding](https://ai.google.dev/gemini-api/docs/video-understanding).
...
La banque **Button Accordion HN** de FreePats a été enregistrée sur un véritable accordéon Hohner par Jeff Stauffer, préparée par michael02022 et publiée sous la dédicace domaine public [CC0 1.0](https://freepats.zenvoid.org/Organ/accordion.html). Les fichiers sources et leur licence sont conservés dans `public/audio/accordion/freepats-hn-20240329`.

Soufflet assume pleinement l’usage pédagogique des œuvres **traditionnelles et du domaine public** : leur mélodie, leur harmonie, leur partition reconstituée et leurs paroles peuvent être intégrées en entier à la bibliothèque commune. Chaque morceau conserve sa provenance et distingue l’œuvre d’un arrangement ou d’une gravure moderne. Le moteur de recherche ne doit donc pas rejeter par excès de prudence une chanson dont le domaine public est établi ; il convertit les faits musicaux dans le format interne sans reproduire la mise en page d’une édition récente.

En droit français, la durée patrimoniale ordinaire court pendant la vie de l’auteur puis les 70 années civiles suivant son décès ; les œuvres de collaboration se calculent depuis le dernier coauteur, avec des cas particuliers à contrôler. Référence : [Code de la propriété intellectuelle, articles L123-1 et suivants](https://www.legifrance.gouv.fr/codes/id/LEGISCTA000006161638). Les œuvres ou arrangements encore protégés ne sont pas injectés dans la bibliothèque commune. Une source fournie par un utilisateur reste dans sa bibliothèque personnelle et n’est jamais promue automatiquement en contenu commun.

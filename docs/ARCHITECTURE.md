...
## Catalogue multi-instruments

Un `Song` représente l’identité commune du morceau, ses sources, ses droits et sa structure temporelle. Il n’est jamais dupliqué par instrument. Les adaptations vivent dans `Song.arrangements`, indexées par `accordion`, `piano` ou `guitar` : chaque arrangement possède ses événements, ses doigtés, ses mains ou cordes et sa provenance. Les anciens événements accordéon restent au premier niveau pendant la migration afin que les clients déjà installés continuent de fonctionner.

Le piano est le premier consommateur de ce contrat. Les mélodies vérifiées deviennent une piste main droite et l’accompagnement une piste main gauche indépendante, avec basses et renversements simples. Le lecteur sait travailler chaque main ou les deux, et accepte trois entrées : MIDI polyphonique, microphone monophonique et clavier/toucher.

La guitare transforme la même mélodie en positions corde/case et l’harmonie en formes d’accords sur une seconde piste. Ces positions sont recalculées depuis les hauteurs MIDI pour l’accordage, le capodastre et le sens du manche configurés. Le micro YIN valide les notes isolées ; le chroma spectral estime les triades sur les attaques d’accords. L’accordeur chromatique compare la fréquence à chacune des cordes de la configuration active et ne dépend donc pas d’un accordage EADGBE codé dans l’interface. Une séance et l’instrument actif sont enregistrés avec leur `instrument_type`, ce qui permet de partager le compte et la bibliothèque tout en gardant des statistiques interprétables.
...
L’instrument actif, ses configurations, la notation, le décompte et l’achèvement de l’onboarding et du tutoriel sont synchronisés dans `user_preferences` et `instrument_configs`. Une copie locale est isolée par identifiant de compte et sert uniquement de repli lorsque le serveur a déjà été joint au moins une fois sur cet appareil. Le brouillon de la première leçon suit la même isolation par identifiant de compte ; l’ancienne clé locale globale est migrée une fois puis supprimée. Les marqueurs d’achèvement sont monotones : une réponse tardive provenant d’un autre appareil ne peut pas réactiver le tutoriel. À la migration, les comptes qui possèdent déjà une séance active sont considérés comme ayant terminé le tutoriel. Les morceaux importés sont synchronisés dans `user_songs` et conservent une copie locale de reprise. Le passage long terme prévu pour l’édition concurrente est un journal d’opérations versionné côté serveur avec IndexedDB comme outbox, identifiants idempotents et résolution de conflits.
